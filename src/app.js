...
function createManager (managerName, managerAge, currentTeam, trophiesWon){
  let CurrentmanagersDetails=[managerName,managerAge,currentTeam,trophiesWon];
  return CurrentmanagersDetails;
...
try {
  var ObjectFormation = FormationDetails(formation);
} catch (e) {
  //do nothing
}
...
function filterByAward(awardname) {
  return players.filter(function(item) {
    return item.awards.some(function(e) {
...
function filterByAwardxTimes(awardname, noOfTimes) {
  return players.filter(function(item) {
    let awardCount = item.awards.reduce(function(acc, curr) {
      return curr.name === awardname ? acc + 1 : acc;
    }, 0);
